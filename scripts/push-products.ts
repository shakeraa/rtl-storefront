#!/usr/bin/env node
/**
 * Push Rich Content Products to Shopify Store
 * 
 * Usage: npx tsx scripts/push-products.ts
 */
...
if (!ACCESS_TOKEN) {
  ...
  console.error("   Get your access token from Shopify Admin > Settings > Apps and sales channels > Develop apps");
  process.exit(1);
}
...
async function createProduct(product: typeof richProducts[0], locationId: string) {
  const query = `
...
async function getLocationId() {
  ...
  try {
    const response = await shopify.clients.Graphql.exec({ session, query });
    const result = response.body as any;
...
async function main() {
  console.log("🚀 Starting to push rich content products to Shopify...\n");
  ...
  const results = {
    success: [] as string[],
    failed: [] as { title: string; errors: any[] }[],
  };

  for (let i = 0; i < richProducts.length; i++) {
    const product = richProducts[i];
    console.log(`[${i + 1}/${richProducts.length}] Creating: ${product.title.substring(0, 60)}...`);
    ...
    if (result.success) {
      ...
      console.log(`   ❌ Failed: ${result.errors.map((e: any) => e.message).join(", ")}`);
      results.failed.push({ title: product.title, errors: result.errors });
    }
  ...
  if (results.failed.length > 0) {
    ...
    results.failed.forEach(f => {
      ...
      f.errors.forEach(e => console.log(`     Error: ${e.message || e}`));
    });
  }

  console.log("\n🎉 All done! Check your Shopify admin to see the products.");
}
