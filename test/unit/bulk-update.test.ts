...
describe("bulk-update handling", () => {
  // Create fresh mock translators for each test to avoid state pollution
  ...
  const createFailingTranslator = () => {
    const mock = vi.fn(async ({ text }): Promise<TranslationResult> => {
      if (text === "fail") {
    ...
    return { translate: mock };
  };

  const createNetworkErrorTranslator = () => ({
    translate: vi.fn(async ({ text }): Promise<TranslationResult> => {
      if (text === "network") {
  ...
  it("handles partial failures correctly", async () => {
    const mockFailingTranslator = createFailingTranslator();
    const job = queueBulkUpdate({
      shop: "test-shop.myshopify.com",
  ...
  it("classifies network errors as retryable", async () => {
    const mockNetworkErrorTranslator = createNetworkErrorTranslator();
    const job = queueBulkUpdate({
      shop: "test-shop.myshopify.com",
  ...
  it("tracks processing history for ETA calculation", async () => {
    const mockTranslator = createMockTranslator();
    // Add a delay to the translator to ensure processing time > 0
    mockTranslator.translate.mockImplementation(async ({ text }) => {
    ...
    const status = getBulkUpdateStatus(job.id);
    // The averageProcessingTime might be 0 if processing is very fast in test
    // Just verify it exists and is a number
