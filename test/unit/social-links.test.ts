...
describe("Social Links - Labels", () => {
  ...
  describe("getSocialAriaLabel", () => {
    it("returns English aria-label for Facebook", () => {
      expect(getSocialAriaLabel("facebook", "en")).toBe(
        "Visit our Facebook page"
      );
    });

    it("returns Arabic aria-label for YouTube", () => {
      expect(getSocialAriaLabel("youtube", "ar")).toBe(
        "زيارة قناتنا على يوتيوب"
      );
    });

    it("returns Hebrew aria-label for Instagram", () => {
      expect(getSocialAriaLabel("instagram", "he")).toBe(
        "בקרו בפרופיל האינסטגרם שלנו"
      );
    });

    it("returns correct Arabic aria-labels for all platforms", () => {
      expect(getSocialAriaLabel("twitter", "ar")).toBe(
        "زيارة ملفنا على تويتر"
      );
      expect(getSocialAriaLabel("tiktok", "ar")).toBe(
        "زيارة ملفنا على تيك توك"
      );
      expect(getSocialAriaLabel("snapchat", "ar")).toBe(
        "زيارة ملفنا على سناب شات"
      );
    });

    it("returns correct Hebrew aria-labels for all platforms", () => {
      expect(getSocialAriaLabel("linkedin", "he")).toBe(
        "בקרו בעמוד הלינקדאין שלנו"
      );
      expect(getSocialAriaLabel("pinterest", "he")).toBe(
        "בקרו בפרופיל הפינטרסט שלנו"
      );
    });

    it("handles locale with region subtag for aria-labels", () => {
      expect(getSocialAriaLabel("facebook", "ar-SA")).toBe(
        "زيارة صفحتنا على فيسبوك"
      );
      expect(getSocialAriaLabel("twitter", "he-IL")).toBe(
        "בקרו בפרופיל הטוויטר שלנו"
      );
    });

    it("falls back to English aria-label for unknown locale", () => {
      expect(getSocialAriaLabel("linkedin", "unknown")).toBe(
  ...
  describe("getAllSocialLinks", () => {
    ...
    it("returns Hebrew labels in getAllSocialLinks", () => {
      ...
      expect(instagramLink?.ariaLabel).toBe(
        "בקרו בפרופיל האינסטגרם שלנו"
      );
    });
