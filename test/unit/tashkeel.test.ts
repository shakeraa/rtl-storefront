...
import {
  ...
  TASHKEEL_CHARS, TASHKEEL_PATTERN
...
describe('Tashkeel Service', () => {
  ...
  describe('normalizeTashkeel', () => {
    it('removes duplicate consecutive tashkeel', () => {
      const text = 'مرحبًًا'; // Double fathatan (same marks)
  ...
  describe('countTashkeel', () => {
    it('counts tashkeel correctly', () => {
      // 4 tashkeel: fatha, sukun, fatha, fathatan
  ...
  describe('TASHKEEL_PATTERN regex', () => {
    it('matches fatha characters', () => {
      const freshPattern = /[\u064B-\u065F\u0670\u0640\uFC60-\uFC62\u06E9-\u06ED]/;
      expect(freshPattern.test('َ')).toBe(true);
    });
    it('matches damma characters', () => {
      const freshPattern = /[\u064B-\u065F\u0670\u0640\uFC60-\uFC62\u06E9-\u06ED]/;
      expect(freshPattern.test('ُ')).toBe(true);
    });
    it('does not match Arabic letters', () => {
      const freshPattern = /[\u064B-\u065F\u0670\u0640\uFC60-\uFC62\u06E9-\u06ED]/;
      expect(freshPattern.test('م')).toBe(false);
    });
    it('does not match Latin letters', () => {
      const freshPattern = /[\u064B-\u065F\u0670\u0640\uFC60-\uFC62\u06E9-\u06ED]/;
      expect(freshPattern.test('a')).toBe(false);
    });
  });
