...
describe("code-splitting", () => {
  describe("getRouteSplitPoints", () => {
    ...
    it("sorts routes by priority (critical first)", () => {
      // Use weights that clearly map to different priorities
      const routes: RouteDefinition[] = [
        { id: "critical", path: "/critical", component: "CritPage", weight: 0.9 },
        { id: "medium", path: "/medium", component: "MedPage", weight: 0.5 },
        { id: "deferred", path: "/deferred", component: "DefPage", weight: 0.05 },
      ];
    ...
    it("assigns appropriate preload triggers based on priority", () => {
      // weight > 0.8 = critical (immediate), 0.1-0.8 = medium/high (viewport/idle), < 0.1 = deferred
      const routes: RouteDefinition[] = [
      ...
      expect(result.find((r) => r.routeId === "deferred")?.preloadTrigger).toBeUndefined();
    });
  });
  ...
  describe("analyzeComponentSplits", () => {
    ...
    it("identifies shared components across multiple routes", () => {
      const components: ComponentMetadata[] = [
        {
          ...
          usedIn: ["/", "/products", "/cart", "/checkout", "/account", "/profile"],
          isLazyLoadable: true,
          priority: "high",
        },
      ];
      ...
      // The component is used in 6 routes (>5 threshold)
      expect(result[0].splitReasons.some((r) => r.includes("6") || r.includes("routes"))).toBe(true);
    });
  ...
  describe("getPrefetchStrategy", () => {
    ...
    it("returns eager strategy for frequent visitors with high engagement", () => {
      ...
      const behavior: UserBehavior = {
        visitedRoutes: ["/custom-route", "/custom-route", "/custom-route", "/custom-route", "/other"],
        averageTimeOnPage: 10000, // High engagement
        clickPattern: ["/custom-route"],
        scrollDepth: 0.5,
        connectionHistory: [],
      };
