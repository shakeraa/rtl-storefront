...
describe('Dialects Service - T0066', () => {
  describe('Dialect Configurations', () => {
    ...
    it('should have correct Levant config', () => {
      const c = getDialectConfig('levant');
    ...
    it('should have correct Maghreb config', () => {
      const c = getDialectConfig('maghreb');
  ...
  describe('Country Detection', () => {
    ...
    it('detects Levant for SY', () => expect(detectDialectFromCountry('SY')).toBe('levant'));
    it('detects Levant for JO', () => expect(detectDialectFromCountry('JO')).toBe('levant'));
    it('detects Maghreb for MA', () => expect(detectDialectFromCountry('MA')).toBe('maghreb'));
    it('detects Egyptian for EG', () => expect(detectDialectFromCountry('EG')).toBe('egyptian'));
    it('defaults to standard for unknown', () => expect(detectDialectFromCountry('XX')).toBe('standard'));
  ...
  describe('Text Detection', () => {
    ...
    it('detects Levant from شو', () => { const r = detectDialect('شو بدك'); expect(r.dialect).toBe('levant'); expect(r.markers).toContain('شو'); });
    it('detects Maghreb from واش', () => { const r = detectDialect('واش تبغي'); expect(r.dialect).toBe('maghreb'); expect(r.markers).toContain('واش'); });
    it('detects Egyptian from إزاي', () => { const r = detectDialect('إزاي'); expect(r.dialect).toBe('egyptian'); expect(r.markers).toContain('إزاي'); });
    it('returns standard for empty', () => { const r = detectDialect(''); expect(r.dialect).toBe('standard'); expect(r.confidence).toBe(0); });
  ...
  describe('Vocabulary', () => {
    ...
    it('gets Levant vocabulary', () => expect(getDialectVocabulary('levant').hello).toBe('مرحبتين'));
  ...
  describe('Translations', () => {
    it('gets greetings', () => { expect(getGreeting('gulf')).toBe('هلا'); expect(getGreeting('levant')).toBe('مرحبتين'); expect(getGreeting('maghreb')).toBe('سلام'); expect(getGreeting('egyptian')).toBe('أهلاً'); });
    it('translates text', () => expect(typeof translateToDialect('test', 'gulf')).toBe('string'));
    it('returns original for standard', () => { const t = 'مرحباً'; expect(translateToDialect(t, 'standard')).toBe(t); });
  ...
  describe('Regions', () => {
    ...
    it('gets Levant regions', () => { const r = getDialectRegions('levant'); expect(r.some(x => x.country === 'SY')).toBe(true); expect(r.some(x => x.country === 'JO')).toBe(true); });
    it('gets Maghreb regions', () => { const r = getDialectRegions('maghreb'); expect(r.some(x => x.country === 'MA')).toBe(true); });
  ...
  describe('Available Dialects', () => {
    it('for SA', () => { const d = getAvailableDialects('SA'); expect(d).toContain('gulf'); expect(d).toContain('standard'); });
    it('for EG', () => { const d = getAvailableDialects('EG'); expect(d).toContain('egyptian'); expect(d).toContain('standard'); });
    it('for unknown', () => expect(getAvailableDialects('XX')).toEqual(['standard']));
  });
  ...
  describe('Phrases', () => {
    ...
    it('gets Levant phrases', () => { expect(getDialectPhrase('levant', 'welcome_phrase')).toBe('أهلاً وسهلاً'); expect(getDialectPhrase('levant', 'hello_response')).toBe('أهلاً فيك'); });
    it('has phrases for all dialects', () => ['gulf', 'levant', 'maghreb', 'egyptian', 'standard'].forEach(d => { expect(getDialectPhrase(d as any, 'welcome_phrase')).toBeDefined(); }));
  });

  describe('Format', () => {
    it('formats Gulf', () => expect(formatDialectName('gulf', 'en')).toBe('Gulf Arabic'));
    it('formats Levant', () => expect(formatDialectName('levant', 'en')).toBe('Levantine Arabic'));
    it('formats in Arabic', () => { expect(formatDialectName('gulf', 'ar')).toBe('خليجي'); expect(formatDialectName('levant', 'ar')).toBe('شامي'); });
  ...
  describe('Term Detection', () => {
    ...
    it('detects Levant terms', () => { expect(containsDialectTerms('kifak', 'levant')).toBe(true); expect(containsDialectTerms('sho', 'levant')).toBe(true); });
    it('detects Maghreb terms', () => { expect(containsDialectTerms('labas', 'maghreb')).toBe(true); expect(containsDialectTerms('shkon', 'maghreb')).toBe(true); });
  ...
  describe('Statistics', () => {
    ...
    it('stats differ', () => { expect(getDialectStats('gulf').countryCount).not.toBe(getDialectStats('levant').countryCount); });
  });
  ...
  describe('Suggestions', () => {
    ...
    it('defaults to standard', () => { const s = suggestDialect(); expect(s.dialect).toBe('standard'); expect(s.confidence).toBeLessThan(0.5); });
  });

  describe('Validation', () => {
    it('validates valid codes', () => { expect(isValidDialect('gulf')).toBe(true); expect(isValidDialect('levant')).toBe(true); expect(isValidDialect('maghreb')).toBe(true); expect(isValidDialect('egyptian')).toBe(true); expect(isValidDialect('standard')).toBe(true); });
  ...
  describe('Regional Variations', () => {
    ...
    it('has secondary dialects', () => Object.values(DIALECT_REGIONS).forEach(r => { expect(r.secondaryDialects).toBeDefined(); expect(r.secondaryDialects).toContain('standard'); }));
  });
});
