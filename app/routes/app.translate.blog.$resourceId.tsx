...
// ---------------------------------------------------------------------------
// Loader — fetch blog article translatable content
// ---------------------------------------------------------------------------
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const shopifyResourceId = `gid://shopify/OnlineStoreArticle/${resourceId}`;
  ...
  // Build source fields with digests
  ...
  // Try to get the article title for the page header
  const articleTitle = sourceFields.find(
    (f: { key: string }) => f.key === "title",
  )?.sourceValue ?? `Blog Article ${resourceId}`;
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
    ...
    for (const field of fields) {
      ...
      try {
        const result = await engine.translate({
          ...
          targetLocale,
          context: `Shopify blog article field: ${field.key}`,
          format: field.key === "body_html" || field.key === "summary_html" ? "html" : "text",
        });
  ...
  // ----- Intent: translate_field — AI-translate a single field -----
  if (intent === "translate_field") {
    ...
    try {
      const result = await engine.translate({
        ...
        sourceLocale,
        targetLocale,
        context: `Shopify blog article field: ${fieldKey}`,
        format: fieldKey === "body_html" || fieldKey === "summary_html" ? "html" : "text",
      });
  ...
  // ----- Intent: save — save manually edited translations -----
  if (intent === "save") {
...
export default function TranslateBlogArticle() {
  const { resourceId, articleTitle, targetLocale, sourceFields } =
  ...
  const handleLocaleChange = useCallback((value: string) => {
    setLocale(value);
    window.location.href = `/app/translate/blog/${resourceId}?locale=${value}`;
  }, [resourceId]);
  ...
  // When translateAll fetcher completes, apply translated fields
  if (
  ...
  const handleTranslateField = useCallback(
    (field: SourceField) => {
      ...
      fetch(`/app/translate/blog/${resourceId}`, {
        method: "POST",
        body: formData,
      })
        .then((res) => res.json())
  ...
  return (
    <Page
      ...
      title={`Translate Blog: ${articleTitle}`}
    >
      <TitleBar title={`Translate Blog: ${articleTitle}`} />
      <BlockStack gap="500">
                ...
                <BlockStack gap="600">
                  ...
                  {sourceFields.length === 0 && (
                    <Text as="p" variant="bodyMd" tone="subdued">
                      No translatable content found for this blog article.
                    </Text>
                  )}
                </BlockStack>
              </Card>
