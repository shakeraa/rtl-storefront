...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (intent === "translate_all") {
    ...
    for (const field of fields) {
      // Skip fields that already have a translation (unless outdated)
      ...
      try {
        const result = await engine.translate({
          ...
          targetLocale,
          context: `Shopify product field: ${field.key}`,
          format: field.key === "body_html" ? "html" : "text",
        });
  ...
  if (intent === "translate_field") {
    ...
    try {
      const result = await engine.translate({
        ...
        sourceLocale,
        targetLocale,
        context: `Shopify product field: ${fieldKey}`,
        format: fieldKey === "body_html" ? "html" : "text",
      });
...
export default function TranslateResource() {
  ...
  // Track fields that are currently being individually translated
  ...
  // Apply AI translations returned from fetcher to the local state
  ...
  if (
    ...
    // We need to check if we already applied these — use a simple flag
                ...
                <BlockStack gap="600">
                  ...
                  {sourceFields.length === 0 && (
                    <Text as="p" variant="bodyMd" tone="subdued">
                      No translatable content found for this resource.
                    </Text>
                  )}
                </BlockStack>
              </Card>
