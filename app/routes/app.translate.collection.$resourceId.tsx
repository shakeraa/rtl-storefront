...
const GID_PREFIX = "gid://shopify/Collection/";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const collectionTitle = sourceFields.find(
    (f: { key: string }) => f.key === "title",
  )?.sourceValue ?? `Collection ${resourceId}`;
    ...
    for (const field of fields) {
      ...
      try {
        const result = await engine.translate({
          ...
          targetLocale,
          context: `Shopify collection field: ${field.key}`,
  ...
  if (intent === "translate_field") {
    ...
    try {
      const result = await engine.translate({
        ...
        sourceLocale,
        targetLocale,
        context: `Shopify collection field: ${fieldKey}`,
...
export default function TranslateCollection() {
  const { resourceId, collectionTitle, targetLocale, sourceFields } =
  ...
  const handleLocaleChange = useCallback((value: string) => {
    setLocale(value);
    window.location.href = `/app/translate/collection/${resourceId}?locale=${value}`;
  }, [resourceId]);
  ...
  const handleTranslateField = useCallback(
    (field: SourceField) => {
      ...
      fetch(`/app/translate/collection/${resourceId}`, {
        method: "POST",
        body: formData,
      })
        .then((res) => res.json())
  ...
  return (
    <Page
      ...
      title={`Translate Collection: ${collectionTitle}`}
    >
      <TitleBar title={`Translate Collection: ${collectionTitle}`} />
      <BlockStack gap="500">
        {translateAllFetcher.data?.success && (
          <Banner
                ...
                <BlockStack gap="600">
                  ...
                  {sourceFields.length === 0 && (
                    <Text as="p" variant="bodyMd" tone="subdued">
                      No translatable content found for this collection.
                    </Text>
                  )}
                </BlockStack>
              </Card>
