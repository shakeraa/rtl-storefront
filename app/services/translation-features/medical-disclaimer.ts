...
const HEALTH_WARNING_LABELS: Record<SupportedLocale, HealthWarningLabel[]> = {
  ar: [
    { id: 'keep-out-reach', label: 'يُحفظ بعيداً عن متناول الأطفال', description: 'يُحفظ في مكان آمن بعيداً عن الأطفال', icon: 'child-safety', color: '#ef4444' },
    { id: 'pregnancy-warning', label: 'يُنصح باستشارة الطبيب للحوامل', description: 'يجب استشارة الطبيب قبل الاستخدام أثناء الحمل', icon: 'pregnancy', color: '#f97316' },
    { id: 'allergy-alert', label: 'تحذير الحساسية', description: 'قد يسبب ردود فعل تحسسية', icon: 'alert-triangle', color: '#eab308' },
    { id: 'dosage-limit', label: 'لا تتجاوز الجرعة الموصى بها', description: 'الالتزام بالجرعة الموصى بها فقط', icon: 'pill', color: '#dc2626' },
    { id: 'prescription-interaction', label: 'تفاعل مع الأدوية الموصوفة', description: 'استشر الطبيب إذا كنت تتناول أدوية أخرى', icon: 'interaction', color: '#f97316' },
    { id: 'temperature-storage', label: 'يُحفظ في درجة حرارة مناسبة', description: 'يُحفظ بعيداً عن الحرارة والرطوبة', icon: 'thermometer', color: '#3b82f6' },
  ],
  he: [
    { id: 'keep-out-reach', label: 'לשמור הרחק מהישג ידם של ילדים', description: 'יש לשמור במקום בטוח הרחק מילדים', icon: 'child-safety', color: '#ef4444' },
    { id: 'pregnancy-warning', label: 'מומלץ להתייעץ עם רופא לנשים בהריון', description: 'יש להתייעץ עם רופא לפני השימוש בהריון', icon: 'pregnancy', color: '#f97316' },
    { id: 'allergy-alert', label: 'אזהרת אלרגיה', description: 'עלול לגרום תגובות אלרגיות', icon: 'alert-triangle', color: '#eab308' },
    { id: 'dosage-limit', label: 'אין לחרוג מהמינון המומלץ', description: 'להקפיד על המינון המומלץ בלבד', icon: 'pill', color: '#dc2626' },
    { id: 'prescription-interaction', label: 'אינטראקציה עם תרופות במרשם', description: 'התייעץ עם רופא אם אתה נוטל תרופות אחרות', icon: 'interaction', color: '#f97316' },
    { id: 'temperature-storage', label: 'לשמור בטמפרטורה מתאימה', description: 'יש לשמור הרחק מחום ולחות', icon: 'thermometer', color: '#3b82f6' },
  ],
  en: [
    { id: 'keep-out-reach', label: 'Keep out of reach of children', description: 'Store in a safe place away from children', icon: 'child-safety', color: '#ef4444' },
    { id: 'pregnancy-warning', label: 'Consult doctor if pregnant', description: 'Consult a doctor before use during pregnancy', icon: 'pregnancy', color: '#f97316' },
    { id: 'allergy-alert', label: 'Allergy Alert', description: 'May cause allergic reactions', icon: 'alert-triangle', color: '#eab308' },
    { id: 'dosage-limit', label: 'Do not exceed recommended dose', description: 'Follow recommended dosage only', icon: 'pill', color: '#dc2626' },
    { id: 'prescription-interaction', label: 'May interact with prescription drugs', description: 'Consult doctor if taking other medications', icon: 'interaction', color: '#f97316' },
    { id: 'temperature-storage', label: 'Store at proper temperature', description: 'Keep away from heat and moisture', icon: 'thermometer', color: '#3b82f6' },
  ],
};

// Medical claim detection patterns (with global flag for matchAll)
const MEDICAL_CLAIM_PATTERNS: Record<SupportedLocale, Record<MedicalClaimType, RegExp[]>> = {
  ar: {
    treatment: [/يعالج|علاج|شفاء|يُعالج|علاجي/g, /يُساعد في علاج|يُحسّن حالة|يُخفف من/g],
    cure: [/يشفي|شفاء تام|قضاء على|يُزيل|يُنهي/g, /علاج نهائي|حل نهائي|شفاء كامل/g],
    prevention: [/يحمي من|وقاية|يمنع|يُقي|حماية/g, /يُقلل من خطر|يُبعد خطر|يُجنبك/g],
    diagnosis: [/يُشخص|كشف|تشخيص|فحص|يُحدد/g, /يُكشف عن|اكتشاف|تحديد/g],
    guarantee: [/ضمان|مضمون|نتائج مضمونة|100%/g, /يضمن|يؤكد|وعد|نتائج مؤكدة/g],
  },
  he: {
    treatment: [/מטפל|טיפול|ריפוי|מסייע|טיפולי/g, /מסייע בטיפול|משפר את מצב|מקל על/g],
    cure: [/מרפא|ריפוי מלא|מסיים|מסיר|מחסל/g, /טיפול סופי|פתרון סופי|ריפוי שלם/g],
    prevention: [/מגן מפני|מניעה|מונע|שומר|הגנה/g, /מפחית סיכון|מרחיק סיכון|מונע/g],
    diagnosis: [/מאבחן|אבחון|בדיקה|זיהוי|קביעה/g, /מגלה|גילוי|קביעת/g],
    guarantee: [/אחריות|מובטח|תוצאות מובטחות|100%/g, /מבטיח|מאשר|הבטחה|תוצאות ודאיות/g],
  },
  en: {
    treatment: [/treat|treatment|therapy|therapeutic|helps? (with|treat)/gi, /relieves?|alleviates?|reduces? symptoms?/gi],
    cure: [/cure|cures|complete cure|eliminates?|eradicates?/gi, /heals?|healing|full recovery|permanent solution/gi],
    prevention: [/prevents?|preventive|protection|protects?/gi, /reduces? risk|guards against|shields?/gi],
    diagnosis: [/diagnos(?:is|es|e)|detect(?:s|ion)|identifies?/gi, /screen(?:s|ing)|recognizes?|determines?/gi],
    guarantee: [/guarantees?|guaranteed|100%|promises?/gi, /assured|assurance|certain|sure/gi],
  },
};

// Prohibited content patterns
const PROHIBITED_CONTENT_PATTERNS: Record<SupportedLocale, RegExp[]> = {
  ar: [/وعد الشفاء|شفاء مضمون|نتائج مضمونة 100%|شفاء فوري/, /بديل عن الدواء|أفضل من الدواء|لا حاجة للطبيب/, /علاج كل الأمراض|شفاء جميع الأمراض|دواء شامل/],
  he: [/הבטחת ריפוי|ריפוי מובטח|תוצאות מובטחות 100%|ריפוי מיידי/, /תחליף לתרופה|טוב יותר מתרופה|אין צורך ברופא/, /טיפול לכל המחלות|ריפוי כל המחלות|תרופה כוללת/],
  en: [/promise.*cure|guaranteed cure|100% guaranteed|instant cure/i, /replacement for medicine|better than medicine|no need for doctor/i, /cure all diseases|cure everything|universal cure/i],
};

// Medical keywords that trigger disclaimer requirements
const MEDICAL_KEYWORDS: Record<SupportedLocale, string[]> = {
  ar: ['طبيب', 'علاج', 'دواء', 'صحة', 'مرض', 'ألم', 'أعراض', 'تشخيص', 'صيدلي', 'مستشفى', 'طوارئ', 'جرعة', 'وصفة طبية', 'مكمل غذائي', 'جهاز طبي', 'فحص', 'تحليل', 'جراحة', 'علاج طبيعي'],
  he: ['רופא', 'טיפול', 'תרופה', 'בריאות', 'מחלה', 'כאב', 'תסמינים', 'אבחון', 'רוקח', 'בית חולים', 'חירום', 'מינון', 'מרשם', 'תוסף תזונה', 'מכשיר רפואי', 'בדיקה', 'ניתוח', 'פיזיותרפיה'],
  en: ['doctor', 'treatment', 'medicine', 'health', 'disease', 'pain', 'symptoms', 'diagnosis', 'pharmacist', 'hospital', 'emergency', 'dosage', 'prescription', 'supplement', 'medical device', 'examination', 'surgery', 'physical therapy'],
};

/**
 * Get medical disclaimer for a specific locale and type
 */
export function getMedicalDisclaimer(locale: SupportedLocale, type: DisclaimerType): MedicalDisclaimer {
 ...
 */
export function validateMedicalContent(content: string, locale: SupportedLocale): MedicalContentValidation {
  ...
  for (const pattern of prohibitedPatterns) {
    if (pattern.test(content)) {
      issues.push({ type: 'prohibited_content', severity: 'error', message: 'Content contains prohibited medical claims', location: 'content' });
    }
  }
  ...
  if (claimCheck.hasMedicalClaims) {
    ...
    if (hasGuaranteeClaim) {
      issues.push({ type: 'unverified_claim', severity: 'warning', message: 'Guarantee claims require additional verification' });
    }
  }
  ...
  if (claimCheck.riskLevel === 'high' && !content.includes('consult')) {
    issues.push({ type: 'insufficient_warning', severity: 'warning', message: 'High-risk medical content should include consultation warning' });
 ...
 */
export function checkMedicalClaims(content: string, locale: SupportedLocale): MedicalClaimCheck {
    ...
    for (const pattern of typePatterns) {
      ...
      for (const match of matches) {
        if (match.index !== undefined) {
          detectedClaims.push({ type: claimType as MedicalClaimType, text: match[0], position: match.index, confidence: 0.85 });
        }
      }
    }
  }
  ...
  return { hasMedicalClaims: detectedClaims.length > 0, detectedClaims, riskLevel, requiresRegulatoryReview };
}
 ...
 */
export function getHealthWarningLabel(locale: SupportedLocale, labelId: string): HealthWarningLabel | undefined {
 ...
 */
export function getDisclaimerText(locale: SupportedLocale, type: DisclaimerType, version: 'full' | 'short' = 'short'): string {
 ...
 */
export function getProductDisclaimers(productType: 'supplement' | 'device' | 'general' | 'medication', locale: SupportedLocale): MedicalDisclaimer[] {
 ...
 */
export function formatDisclaimerForDisplay(disclaimer: MedicalDisclaimer, options: { showTitle?: boolean; showIcon?: boolean } = {}): { text: string; title: string; isRTL: boolean; warningLevel: WarningLevel } {
  const { showTitle = true } = options;
  const isRTL = disclaimer.locale === 'ar' || disclaimer.locale === 'he';

  return { text: disclaimer.content, title: showTitle ? disclaimer.title : '', isRTL, warningLevel: disclaimer.warningLevel };
}
...
export function getAllMedicalDisclaimers(locale: SupportedLocale): MedicalDisclaimer[] {
  return (Object.keys(MEDICAL_DISCLAIMERS) as DisclaimerType[]).map(type => getMedicalDisclaimer(locale, type));
}
 ...
 */
export function getDisclaimerRequirements(contentType: 'product-description' | 'marketing' | 'educational' | 'packaging'): { requiredTypes: DisclaimerType[]; requiresAcknowledgment: boolean; minWarningLevel: WarningLevel } {
  switch (contentType) {
    case 'packaging':
      return { requiredTypes: ['general', 'consult-doctor'], requiresAcknowledgment: true, minWarningLevel: 'warning' };
    case 'marketing':
      return { requiredTypes: ['general', 'not-medical-advice'], requiresAcknowledgment: false, minWarningLevel: 'info' };
    case 'educational':
      return { requiredTypes: ['not-medical-advice'], requiresAcknowledgment: false, minWarningLevel: 'info' };
    case 'product-description':
    default:
      return { requiredTypes: ['general', 'product'], requiresAcknowledgment: true, minWarningLevel: 'warning' };
  }
}
