...
export function convertUnit(value: number, fromUnit: string, toUnit: string): ConversionResult {
  ...
  if (from.category !== to.category) {
    throw new Error(`Incompatible units: cannot convert ${from.category} to ${to.category}`);
  }
...
export function convertTemperature(
  ...
  if (fromScale === toScale) {
    return { value, fromUnit: fromScale, toUnit: toScale, originalValue: value };
  }

  let result: number;
  if (fromScale === 'celsius') {
    result = toScale === 'fahrenheit' ? convertCelsiusToFahrenheit(value) : convertCelsiusToKelvin(value);
  } else if (fromScale === 'fahrenheit') {
    result = toScale === 'celsius' ? convertFahrenheitToCelsius(value) : convertFahrenheitToKelvin(value);
  } else {
    result = toScale === 'celsius' ? convertKelvinToCelsius(value) : convertKelvinToFahrenheit(value);
  }
...
export function getUnitLabel(unit: string, locale: Locale = 'en', plural: boolean | PluralForm = false): string {
  ...
  if (['celsius', 'fahrenheit', 'kelvin'].includes(unitCode)) {
    ...
    if (form === 'dual' && translation.dual) return translation.dual;
...
export function getUnitSymbol(unit: string, locale: Locale = 'en'): string {
  const unitCode = unit.toLowerCase();
  const translations = ['celsius', 'fahrenheit', 'kelvin'].includes(unitCode)
    ? TEMPERATURE_TRANSLATIONS[unitCode]
    : UNIT_TRANSLATIONS[unitCode];
  if (!translations) return unitCode;
...
export function formatNumber(value: number, locale: Locale, decimals: number = 2): string {
  ...
  switch (locale) {
    ...
    case 'he':
      return rounded.toLocaleString('he-IL', { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
    default:
...
export function smartConvert(value: number, fromUnit: string, preferredSystem: 'metric' | 'imperial' = 'metric'): { value: number; unit: string; original: ConversionResult } {
  ...
  if (unit.system === preferredSystem) {
    return { value, unit: fromUnit, original: { value, fromUnit, toUnit: fromUnit, originalValue: value } };
  }
...
export function parseMeasurement(input: string): { value: number; unit: string } | null {
  const match = input.trim().match(/^(\d[\d٠١٢٣٤٥٦٧٨٩,.٫]*)\s*(.+)?$/);
...
export function calculatePercentageDifference(value1: number, unit1: string, value2: number, unit2: string): number {
  if (!areUnitsCompatible(unit1, unit2)) {
    throw new Error(`Cannot calculate difference for incompatible units: ${unit1} and ${unit2}`);
  }
  const converted = convertUnit(value1, unit1, unit2);
  if (value2 === 0) return 0;
  return ((converted.value - value2) / Math.abs(value2)) * 100;
}
