 ...
 * 
 * Provides localized color names for Arabic (ar), Hebrew (he), and English (en) locales.
 * Includes support for color shades (light, dark, etc.)
 */
...
/**
 * Get color name translation for a specific locale
 * @param color - The color key (e.g., 'red', 'blue')
 * @param locale - The locale code (e.g., 'ar', 'he', 'en')
 * @returns The translated color name or the original color key if not found
 */
...
/**
 * Get a list of all available color names for a specific locale
 * @param locale - The locale code (e.g., 'ar', 'he', 'en')
 * @returns Array of objects containing color key, name, and hex value
 */
export function getColorNamesList(locale: string): Array<{
  key: string;
  name: string;
  hex: string;
  hasShades: boolean;
...
/**
 * Get color name with shade modifier
 * @param color - The color key (e.g., 'red', 'blue')
 * @param shade - The shade type (e.g., 'light', 'dark')
 * @param locale - The locale code (e.g., 'ar', 'he', 'en')
 * @returns The translated color name with shade, or base color name if shade not available
 */
export function getColorWithShade(
  color: string,
  shade: ColorShade,
  locale: string
): string {
  ...
  // Fallback to English shade pattern if specific shade not available
  const englishColor = ENGLISH_COLORS[colorKey];
  if (englishColor?.shades?.[shade]) {
    // Return base name if shade not available in target locale
...
/**
 * Get all available shades for a specific color
 * @param color - The color key (e.g., 'red', 'blue')
 * @param locale - The locale code (e.g., 'ar', 'he', 'en')
 * @returns Array of available shade objects
 */
export function getColorShades(
  color: string,
  locale: string
): Array<{ shade: ColorShade; name: string }> {
  const normalizedLocale = normalizeLocale(locale);
  const colorSet = COLORS_BY_LOCALE[normalizedLocale];
  const colorKey = color.toLowerCase();
  const colorData = colorSet[colorKey];
  ...
  return Object.entries(colorData.shades).map(([shade, name]) => ({
...
/**
 * Check if a color supports shades
 * @param color - The color key (e.g., 'red', 'blue')
 * @returns Boolean indicating if the color has shade variants
 */
 ...
 * Get color hex code
 * @param color - The color key (e.g., 'red', 'blue')
 * @returns The hex code for the color or null if not found
 */
...
/**
 * Search colors by name (partial match)
...
export function searchColors(
  query: string,
  locale: string
): Array<{ key: string; name: string; hex: string }> {
  const normalizedLocale = normalizeLocale(locale);
  const colorSet = COLORS_BY_LOCALE[normalizedLocale];
...
export function getColorsByCategory(locale: string): {
  ...
  const mapColors = (keys: string[]) =>
    keys
      .filter(key => colorSet[key])
      .map(key => ({
        key,
        name: colorSet[key].name,
        hex: colorSet[key].hex,
      }));
  
  return {
    basic: mapColors(basicColors),
    bright: mapColors(brightColors),
    neutral: mapColors(neutralColors),
    metallic: mapColors(metallicColors),
  };
}
