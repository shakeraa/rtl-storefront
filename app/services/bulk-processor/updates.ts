...
export async function retryFailedItems(
  ...
  // Reset job status to queued first (processBulkUpdate will set to processing)
  job.status = "queued";
  job.error = undefined;
  job.updatedAt = new Date();
...
export async function resumeBulkUpdate(
  ...
  // Reset to queued so processBulkUpdate can set it to processing
  job.status = "queued";
  job.resumedAt = new Date();
  job.updatedAt = new Date();

  return processBulkUpdate(job.id, translator);
}
