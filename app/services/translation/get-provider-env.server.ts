...
export async function getProviderEnv(shop: string): Promise<TranslationServiceEnv> {
  ...
  return {
    // DB keys take precedence over env vars — decrypt DB-stored keys
    OPENAI_API_KEY: decrypt(settings?.openaiApiKey ?? "") || process.env.OPENAI_API_KEY,
    ...
    DEEPL_API_KEY: decrypt(settings?.deeplApiKey ?? "") || process.env.DEEPL_API_KEY,
    ...
    AZURE_TRANSLATOR_KEY: decrypt(settings?.azureTranslatorKey ?? "") || process.env.AZURE_TRANSLATOR_KEY,
    AZURE_TRANSLATOR_REGION: decrypt(settings?.azureTranslatorRegion ?? "") || process.env.AZURE_TRANSLATOR_REGION,
    ...
    AWS_ACCESS_KEY_ID: decrypt(settings?.awsAccessKeyId ?? "") || process.env.AWS_ACCESS_KEY_ID,
    AWS_SECRET_ACCESS_KEY: decrypt(settings?.awsSecretAccessKey ?? "") || process.env.AWS_SECRET_ACCESS_KEY,
    AWS_REGION: decrypt(settings?.awsRegion ?? "") || process.env.AWS_REGION,
    ...
    ANTHROPIC_API_KEY: decrypt(settings?.anthropicApiKey ?? "") || process.env.ANTHROPIC_API_KEY,
    ...
    LIBRETRANSLATE_URL: decrypt(settings?.libreTranslateUrl ?? "") || process.env.LIBRETRANSLATE_URL,
    LIBRETRANSLATE_API_KEY: decrypt(settings?.libreTranslateApiKey ?? "") || process.env.LIBRETRANSLATE_API_KEY,
