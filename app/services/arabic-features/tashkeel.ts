...
export const TASHKEEL_CHARS = {
  ...
  MADDAH_ABOVE: '\u0653', HAMZA_ABOVE: '\u0654', HAMZA_BELOW: '\u0655',
} as const;
...
function isArabicLetter(char: string): boolean {
  ...
  if (code >= 0x0600 && code <= 0x06FF) {
    if ((code >= 0x0600 && code <= 0x061F) || code === 0x0640) return false;
    return true;
  }
...
function getTashkeelPositionsByType(text: string, type: TashkeelType): number[] {
  ...
  for (let i = 0; i < chars.length; i++) {
    if (getTashkeelType(chars[i]) === type) positions.push(i);
  }
  return positions;
}
...
export function isTashkeelChar(char: string, includeExtended = false): boolean {
  ...
  if (code === 0x0670 || code === 0x0640) return true;
  if (includeExtended) {
